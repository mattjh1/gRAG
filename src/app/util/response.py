...
async def display_response_with_thinking_step(response_content: str, step_name: str = "Analysis"):
    """Display response with thinking as a separate chainlit step"""
    
    thinking, answer = ResponseParser.extract_think_and_answer(response_content)
    
    if thinking:
        step_title = f"🧠 {step_name} - Reasoning" if "Reasoning" not in step_name else f"🧠 {step_name}"
